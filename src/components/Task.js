...
const Task = () => {
  ...
  const get_Task = async() => {
    const response = await fetch(`https://django-api.up.railway.app/api/todo/${params.id}/`,{
      method:'GET',
      headers:{
  ...
  const delete_note = async() => {
    const response = await fetch(`https://django-api.up.railway.app/api/todo/delete/${params.id}/`,{
      method:'DELETE',
      headers:{
          'Content-Type':'application/json',
          'Authorization':(`Bearer ${token.access}`)
